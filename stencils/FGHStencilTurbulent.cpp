#include "FGHStencilTurbulent.h"
#include "StencilFunctions.h"
#include "Definitions.h"

FGHStencilTurbulent::FGHStencilTurbulent ( const Parameters & parameters ) : FieldStencil<FlowField> ( parameters ) {}


void FGHStencilTurbulent::apply ( FlowField & flowField,  int i, int j ){
...
void FGHStencilTurbulent::apply ( FlowField & flowField, int i, int j, int k ){
	...
	loadLocalViscosity3D(_parameters, _localViscosity, i, j, k);

        if ((obstacle & OBSTACLE_RIGHT) == 0) { // If the right cell is fluid
	  values [0] = computeF3D(_localVelocity, _localMeshsize, _localViscosity, _parameters, _parameters.timestep.dt);
        }
        if ((obstacle & OBSTACLE_TOP) == 0) {
	  values [1] = computeG3D(_localVelocity, _localMeshsize, _localViscosity, _parameters, _parameters.timestep.dt);
        }
        if ((obstacle & OBSTACLE_BACK) == 0) {
	  values [2] = computeH3D(_localVelocity, _localMeshsize, _localViscosity, _parameters, _parameters.timestep.dt);
        }
    }
}
