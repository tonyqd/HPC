...
void VelocityBufferFillStencil::applyLeftWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */
  leftVelocityFillBuffer[1 + 3 * ((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i+1, j, k)[0];
  leftVelocityFillBuffer[2 + 3 * ((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i+1, j, k)[1];
  leftVelocityFillBuffer[3 + 3 * ((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i+1, j, k)[2];
...
void VelocityBufferFillStencil::applyRightWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */
  rightVelocityFillBuffer[0 + 4*((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i-1,j,k)[0];
  rightVelocityFillBuffer[1 + 4*((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i-1,j,k)[1];
  rightVelocityFillBuffer[2 + 4*((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i-1,j,k)[2];
  rightVelocityFillBuffer[3 + 4*((k-1)+(localSize[1]*(j-1)))] = getVelocity().getVector(i-2,j,k)[0];
}
...
void VelocityBufferFillStencil::applyBottomWall ( FlowField & flowField, int i, int j, int k) {
   ...
   * from ParallelBoundaryIterater:  i = _lowOffset ; j = _lowOffset ; k = _lowOffset;
   */
  bottomVelocityFillBuffer[0 + 3*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j+1,k)[0];
  bottomVelocityFillBuffer[1 + 3*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j+1,k)[1];
  bottomVelocityFillBuffer[2 + 3*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j+1,k)[2];
}
...
void VelocityBufferFillStencil::applyTopWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */
  topVelocityFillBuffer[0 + 4*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j-1,k)[0];
  topVelocityFillBuffer[1 + 4*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j-1,k)[1];
  topVelocityFillBuffer[2 + 4*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j-1,k)[2];
  topVelocityFillBuffer[3 + 4*((k-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j-2,k)[1];
}
...
void VelocityBufferFillStencil::applyFrontWall ( FlowField & flowField, int i, int j, int k) {
   ...
   * from ParallelBoundaryIterater:  i = _lowOffset ; j = _lowOffset ; k = _lowOffset;
   */
  frontVelocityFillBuffer[0 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k+1)[0];
  frontVelocityFillBuffer[1 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k+1)[1];
  frontVelocityFillBuffer[2 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k+1)[2];
}
...
void VelocityBufferFillStencil::applyBackWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */
  backVelocityFillBuffer[0 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k-1)[0];
  backVelocityFillBuffer[1 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k-1)[1];
  backVelocityFillBuffer[2 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k-1)[2];
  backVelocityFillBuffer[3 + 4*((j-1)+(localSize[0]*(i-1)))] = getVelocity().getVector(i,j,k-2)[2];
}
