...
PressureBufferFillStencil::PressureBufferFillStencil( const Parameters & parameters):
  ...
  bottomPressureFillBuffer = new FLOAT[(localSize[0]) * (localSize[1])];
  topPressureFillBuffer = new FLOAT[(localSize[0]) * (localSize[1])];

  frontPressureFillBuffer = new FLOAT[(localSize[0]) * (localSize[2])];
  backPressureFillBuffer = new FLOAT[(localSize[0]) * (localSize[2])];
...
void PressureBufferFillStencil::applyLeftWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */
  leftPressureFillBuffer[(k-2)+(localSize[2]*(j-2))] = flowField.getPressure().getScalar(i,j,k);
}
...
void PressureBufferFillStencil::applyRightWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */
  rightPressureFillBuffer[(k-2)+(localSize[2]*(j-2))] = flowField.getPressure().getScalar(i,j,k);
}
...
void PressureBufferFillStencil::applyBottomWall ( FlowField & flowField, int i, int j, int k) {
   ...
   * from ParallelBoundaryIterater:  i = _lowOffset ; j = _lowOffset ; k = _lowOffset;
   */
  bottomPressureFillBuffer[(k-2)+(localSize[2]*(i-2))] = flowField.getPressure().getScalar(i,j,k);
}
...
void PressureBufferFillStencil::applyTopWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */ 
  topPressureFillBuffer[(k-2)+(localSize[2]*(i-2))] = flowField.getPressure().getScalar(i,j,k);
}
...
void PressureBufferFillStencil::applyFrontWall ( FlowField & flowField, int i, int j, int k) {
   ...
   * from ParallelBoundaryIterater:  i = _lowOffset ; j = _lowOffset ; k = _lowOffset;
   */
  frontPressureFillBuffer[(j-2)+(localSize[1]*(i-2))] = flowField.getPressure().getScalar(i,j,k);
}
...
void PressureBufferFillStencil::applyBackWall ( FlowField & flowField, int i, int j, int k) {
   ...
   */   
  backPressureFillBuffer[(j-2)+(localSize[1]*(i-2))] = flowField.getPressure().getScalar(i,j,k);
}
