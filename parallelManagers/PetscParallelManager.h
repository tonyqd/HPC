...
void PetscParallelManager<FlowField>::communicatePressure() {
	...
	MPI_Recv(_pressureBufferReadStencil.rightPressureReadBuffer, (_pressureBufferFillStencil.localSize[1]+3)*(_pressureBufferFillStencil.localSize[2]+3), MY_MPI_FLOAT, _parameters.parallel.rightNb, 0, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
	...
	if(_parameters.geometry.dim ==2) {
		...
		// Copy from left to right
		MPI_Send(_pressureBufferFillStencil.leftPressureFillBuffer, (_pressureBufferFillStencil.localSize[1]+3), MY_MPI_FLOAT, _parameters.parallel.leftNb, 0, PETSC_COMM_WORLD);
		MPI_Recv(_pressureBufferReadStencil.rightPressureReadBuffer, (_pressureBufferFillStencil.localSize[1]+3), MY_MPI_FLOAT, _parameters.parallel.rightNb, 0, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		// Copy from right to left
		MPI_Send(_pressureBufferFillStencil.rightPressureFillBuffer, (_pressureBufferFillStencil.localSize[1]+3)*2, MY_MPI_FLOAT, _parameters.parallel.rightNb, 1, PETSC_COMM_WORLD);
		MPI_Recv(_pressureBufferReadStencil.leftPressureReadBuffer, (_pressureBufferFillStencil.localSize[1]+3)*2, MY_MPI_FLOAT, _parameters.parallel.leftNb, 1, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		_parallelPressureReadBoundaryIterator.iterate();
		_parallelPressureFillBoundaryIterator.iterate();
		// Copy from top to bottom
		MPI_Send(_pressureBufferFillStencil.topPressureFillBuffer, (_pressureBufferFillStencil.localSize[0]+3)*2, MY_MPI_FLOAT, _parameters.parallel.topNb, 2, PETSC_COMM_WORLD);
		MPI_Recv(_pressureBufferReadStencil.bottomPressureReadBuffer, (_pressureBufferFillStencil.localSize[0]+3)*2, MY_MPI_FLOAT, _parameters.parallel.bottomNb, 2, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		// Copy from bottom to top
		MPI_Send(_pressureBufferFillStencil.bottomPressureFillBuffer, (_pressureBufferFillStencil.localSize[0]+3), MY_MPI_FLOAT, _parameters.parallel.bottomNb, 3, PETSC_COMM_WORLD);
		MPI_Recv(_pressureBufferReadStencil.topPressureReadBuffer, (_pressureBufferFillStencil.localSize[0]+3), MY_MPI_FLOAT, _parameters.parallel.topNb, 3, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
		_parallelPressureReadBoundaryIterator.iterate();
	}
...
void PetscParallelManager<FlowField>::communicateVelocities() {
	...
	if(dim ==2){
		...
		// Copy from left to right
		MPI_Send(_velocityBufferFillStencil.leftVelocityFillBuffer, ((_velocityBufferFillStencil.localSize[1] + 3) * dim), MY_MPI_FLOAT, _parameters.parallel.leftNb, 0, PETSC_COMM_WORLD);
		MPI_Recv(_velocityBufferReadStencil.rightVelocityReadBuffer, ((_velocityBufferFillStencil.localSize[1] + 3) * dim), MY_MPI_FLOAT, _parameters.parallel.rightNb, 0, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		// Copy from right to left
		MPI_Send(_velocityBufferFillStencil.rightVelocityFillBuffer, ((_velocityBufferFillStencil.localSize[1] + 3) * 2*(dim )), MY_MPI_FLOAT, _parameters.parallel.rightNb, 1, PETSC_COMM_WORLD);
		MPI_Recv(_velocityBufferReadStencil.leftVelocityReadBuffer, ((_velocityBufferFillStencil.localSize[1] + 3) * 2*(dim )), MY_MPI_FLOAT, _parameters.parallel.leftNb, 1, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
		_parallelVelocityReadBoundaryIterator.iterate();

		// Copy from bottom to top
		_parallelVelocityFillBoundaryIterator.iterate();
		MPI_Send(_velocityBufferFillStencil.bottomVelocityFillBuffer, ((_velocityBufferFillStencil.localSize[0] + 3) *  dim), MY_MPI_FLOAT, _parameters.parallel.bottomNb, 2, PETSC_COMM_WORLD);
		MPI_Recv(_velocityBufferReadStencil.topVelocityReadBuffer, ((_velocityBufferFillStencil.localSize[0] + 3) *  dim), MY_MPI_FLOAT, _parameters.parallel.topNb, 2, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		// Copy from top to bottom
		MPI_Send(_velocityBufferFillStencil.topVelocityFillBuffer, ((_velocityBufferFillStencil.localSize[0] + 3) *  2*(dim)), MY_MPI_FLOAT, _parameters.parallel.topNb, 3, PETSC_COMM_WORLD);
		MPI_Recv(_velocityBufferReadStencil.bottomVelocityReadBuffer, ((_velocityBufferFillStencil.localSize[0] + 3) *  2*(dim )), MY_MPI_FLOAT, _parameters.parallel.bottomNb, 3, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
		_parallelVelocityReadBoundaryIterator.iterate();
	}
...
void PetscParallelManager<FlowField>::communicateViscosity() {
	...
	if (dim == 2){
		...
		// Copy from left to right
		MPI_Send(_viscosityBufferFillStencil.leftViscosityFillBuffer, ((_viscosityBufferFillStencil.localSize[1] + 3) ), MY_MPI_FLOAT, _parameters.parallel.leftNb, 0, PETSC_COMM_WORLD);
		MPI_Recv(_viscosityBufferReadStencil.rightViscosityReadBuffer, ((_viscosityBufferFillStencil.localSize[1] + 3) ), MY_MPI_FLOAT, _parameters.parallel.rightNb, 0, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		// Copy from right to left
		MPI_Send(_viscosityBufferFillStencil.rightViscosityFillBuffer, ((_viscosityBufferFillStencil.localSize[1] + 3) ), MY_MPI_FLOAT, _parameters.parallel.rightNb, 1, PETSC_COMM_WORLD);
		MPI_Recv(_viscosityBufferReadStencil.leftViscosityReadBuffer, ((_viscosityBufferFillStencil.localSize[1] + 3)  ), MY_MPI_FLOAT, _parameters.parallel.leftNb, 1, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
		_parallelViscosityReadBoundaryIterator.iterate();

		// Copy from bottom to top
		_parallelViscosityFillBoundaryIterator.iterate();
		MPI_Send(_viscosityBufferFillStencil.bottomViscosityFillBuffer, ((_viscosityBufferFillStencil.localSize[0] + 3)), MY_MPI_FLOAT, _parameters.parallel.bottomNb, 2, PETSC_COMM_WORLD);
		MPI_Recv(_viscosityBufferReadStencil.topViscosityReadBuffer, ((_viscosityBufferFillStencil.localSize[0] + 3)  ), MY_MPI_FLOAT, _parameters.parallel.topNb, 2, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);

		// Copy from top to bottom
		MPI_Send(_viscosityBufferFillStencil.topViscosityFillBuffer, ((_viscosityBufferFillStencil.localSize[0] + 3) ), MY_MPI_FLOAT, _parameters.parallel.topNb, 3, PETSC_COMM_WORLD);
		MPI_Recv(_viscosityBufferReadStencil.bottomViscosityReadBuffer, ((_viscosityBufferFillStencil.localSize[0] + 3)  ), MY_MPI_FLOAT, _parameters.parallel.bottomNb, 3, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
		_parallelViscosityReadBoundaryIterator.iterate();
	}
